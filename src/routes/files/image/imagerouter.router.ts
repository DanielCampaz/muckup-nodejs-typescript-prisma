...
export default class ImageRouter implements IRouter {
  router: Router;
  path: PathType;
  fileDb: FileDatabase;
  static instance: ImageRouter | null = null;
  ...
  initializeRoutes<T>(_value?: T | undefined): void {
    const multer = new MulterImplementation('IMAGE', {}, {});
    this.router.post(
      '/:id',
      ...
      if (file !== undefined) {
        ...
        if (ets !== false) {
          db.create({
            ...
            .catch((_err) => {
              res.json({ error: 'To Create File to Database {{ImageRouter}}' });
            });
        } else {
          res.json({ error: 'Get Type And Subtype {{ImageRouter}}' });
        }
      }
    };
  }
  ...
  static getInstance(): ImageRouter {
