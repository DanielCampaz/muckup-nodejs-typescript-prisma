...
export default class DocumentRouter implements IRouter {
  ...
  path: PathType;
  fileDb: FileDatabase;
  static instance: DocumentRouter | null = null;
  ...
  initializeRoutes<T>(_value?: T | undefined): void {
    const multer = new MulterImplementation('DOCUMENT', {}, {});
    this.router.post(
      '/:id',
      ...
      if (file !== undefined) {
        ...
        if (ets !== false) {
          db.create({
            ...
            .catch((_err) => {
              res.json({
                error: 'To Create File to Database {{DocumentRouter}}'
              });
            });
        } else {
          res.json({ error: 'Get Type And Subtype {{DocumentRouter}}' });
        }
      }
    };
  }
  ...
  static getInstance(): DocumentRouter {
    if (this.instance === null) {
      this.instance = new DocumentRouter();
    }
